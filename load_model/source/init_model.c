...
void init_model_struct(model_t *model) {
  ...
  // init_list(&model->object_vt_list, sizeof(*model->object_vt_list.data));
  init_list(&model->object_num_vertices, sizeof(*model->object_num_vertices.data));
  init_list(&model->object_vt_index, sizeof(*model->object_vt_index.data));
  init_list(&model->objects_origin, sizeof(*model->objects_origin.data));
  ...
  init_list(&model->material_types, sizeof(*model->material_types.data));
...
void init_textures_struct(textures_t *textures) {
  ...
  init_list(&textures->texture_sizes, sizeof(*textures->texture_sizes.data));
  ...
  init_list(&textures->texture_total_sizes, sizeof(*textures->texture_total_sizes.data));
...
void free_model_memory(model_t *model) {
  ...
  if (model->num_objects) {
    ...
    // free_list(&model->object_vt_list);
    free_list(&model->object_num_vertices);
    free_list(&model->object_vt_index);
    free_list(&model->objects_origin);
  ...
  free_list(&model->material_types);
