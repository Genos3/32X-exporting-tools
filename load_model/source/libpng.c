...
int Save_PNG(const char *filename, unsigned char *buffer, int width, int height, int is_rgba)
{
  png_image image;
  ...
  {
    printf("%s(): png_image_write_to_file(): %s", __func__, image.message);
    return 1;
  }
...
int Read_PNG(const char *filename, unsigned char **_buffer, int *_width, int *_height)
{
  png_image image;
  ...
  {
    printf("%s(): png_image_begin_read_from_file(): %s", __func__, image.message);
    return 1;
  }
